...
namespace TANGOCCONG.ANUIShop.API.Payment.Commons
{
    public class Utils
    {
        ...
        public static string CreateRequestUrl(string url, object _requestData)
        {
            StringBuilder data = new StringBuilder();
            foreach (PropertyInfo item in _requestData.GetType().GetProperties())
            {
                if (!string.IsNullOrEmpty(item.Name))
                    data.Append(item.Name + "=" + HttpUtility.UrlEncode(item.GetValue(_requestData).ToString()) + "&");
            }
            url += "?" + data.ToString();
            return url;
        }
