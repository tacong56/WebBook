...
namespace TANGOCCONG.ANUIShop.API.Controllers
{
    public class UsersController : MyController
    {
        ...
        public async Task<IActionResult> Authenticate(AuthenticateRequest request)
        {
            ...
            var result = await _userService.Authenticate(request, "ADMIN");
        ...
        public async Task<IActionResult> AuthenticateClient(AuthenticateRequest request)
        {
            if (!ModelState.IsValid)
            {
                _logger.LogErr(string.Format("Đăng nhập không thành công với tài khoản {0}", request.UserName));
            ...
            var result = await _userService.Authenticate(request, "CLIENT");
